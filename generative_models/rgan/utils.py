...
def load_settings_from_file(settings):
    ...
    """
    settings_path = './generative_models/rgan/experiments/settings/' + settings['settings_file'] + '.txt'
...
def load_settings_from_dict(settings_expected, settings_dict):
    for key in settings_expected.keys():
        if not key in settings_dict:
            print(key, 'not found in settings_dict - adopting value from command line defaults: ', settings_expected[key])
    settings_expected.update(settings_dict)
    return settings_expected
