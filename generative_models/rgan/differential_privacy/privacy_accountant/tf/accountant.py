...
# pylint: disable=fixme,bad-super-call,unnecessary-pass
...
class MomentsAccountant(object):
    ...
    Dependent on the distribution, we can often obtain a tighter estimation on
    ...
    def __init__(self, total_examples, moment_orders=32):
        ...
        self._total_examples = total_examples
        self._moment_orders = (
            moment_orders if isinstance(moment_orders, (list, tuple)) else range(1, moment_orders + 1))
...
class GaussianMomentsAccountant(MomentsAccountant):
    ...
    Using this we derive that for training T epochs, with batch ratio q,
    ...
    def _compute_log_moment(self, sigma, q, moment_order):
        ...
        Returns:
            log E[exp(moment_order * X)]
        """
        assert moment_order <= self._max_moment_order, (
            "The order of %d is out of the upper bound %d." % (moment_order, self._max_moment_order))
