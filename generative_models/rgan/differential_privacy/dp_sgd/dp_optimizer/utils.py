...
def BuildNetwork(inputs, network_parameters):
    ...
    for conv_param in network_parameters.conv_parameters:
        ...
        conv_bias = tf.Variable(
                tf.truncated_normal([conv_param.out_channels], stddev=conv_param.bias_stddev),
                trainable=conv_param.trainable,
                name=conv_bias_name)
        training_parameters[conv_weights_name] = {}
        training_parameters[conv_bias_name] = {}
        conv = tf.nn.conv2d(outputs, conv_weights, strides=[1, conv_param.stride, conv_param.stride, 1], padding="SAME")
    ...
    for layer_parameters in network_parameters.layer_parameters:
        ...
        hidden_weights = tf.Variable(
                tf.truncated_normal([num_inputs, num_units], stddev=1.0 / math.sqrt(num_inputs)),
        ...
        if layer_parameters.with_bias:
            ...
            if layer_parameters.bias_gradient_l2norm_bound:
                training_parameters[hidden_biases_name]["bias_gradient_l2norm_bound"] = (layer_parameters.bias_gradient_l2norm_bound)
...
def SoftThreshold(t, threshold_ratio, name=None):
    ...
    Returns:
        ...
        threshold_ratio times the mean of the absolute value of each column.
    """
