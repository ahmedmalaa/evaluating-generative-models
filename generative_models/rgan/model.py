...
def sample_Z(batch_size, seq_length, latent_dim, use_time=False, use_noisy_time=False):
    ...
    if use_time:
        ...
        sample[:, :, 0] = np.linspace(0, 1.0/seq_length, num=seq_length)  # pylint: disable=unsupported-assignment-operation
...
def GAN_solvers(
    D_loss, G_loss, learning_rate, batch_size, total_examples, 
...
def dump_parameters(identifier, sess):
    ...
    """
    dump_path = './generative_models/rgan/experiments/parameters/' + identifier + '.npy'
...
def load_parameters(identifier):
    ...
    """
    load_path = './generative_models/rgan/experiments/parameters/' + identifier + '.npy'
...
def sample_trained_model(settings, epoch, num_samples, Z_samples=None, C_samples=None):
    ...
    if type(settings) == str:
        settings = json.load(open('./generative_models/rgan/experiments/settings/' + settings + '.txt', 'r'))
...
def invert(
    settings, epoch, samples, g_tolerance=None, e_tolerance=0.1, 
