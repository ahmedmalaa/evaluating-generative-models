# pylint: disable=unused-argument,fixme,unbalanced-tuple-unpacking,redefined-outer-name
...
def get_samples_and_labels(settings):
    ...
    if settings['data_load_from']:
        data_path = './generative_models/rgan/experiments/data/' + settings['data_load_from'] + '.data.npy'
...
def load_resized_mnist_0_5(new_size, randomize=False):
    """ Load resized mnist digits from 0 to 5 """
...
def resampled_eICU(
    ...
    **kwargs):
    ...
    for (i, patient) in enumerate(pats_grouped.groups):
        ...
        pids.append(patient)
    assert i == n_patients - 1  # pylint: disable=undefined-loop-variable
...
def generate_eICU_resampled_patients(
    resample_factor_in_min=15,
...
def generate_synthetic(identifier, epoch, n_train, predict_labels=False):
    ...
    """
    settings = json.load(open('./generative_models/rgan/experiments/settings/' + identifier + '.txt', 'r'))
    if not settings['cond_dim'] > 0:
        assert settings['predict_labels']
        assert predict_labels
    # get the test data
    print('Loading test (real) data for', identifier)
    data_dict = np.load('./generative_models/rgan/experiments/data/' + identifier + '.data.npy').item()
    ...
    # save it all up
    np.save('./generative_models/rgan/experiments/tstr/' + identifier + '_' + str(epoch) + '.data.npy', exp_data)
    return True
