...
import tensorflow.contrib.layers as tcl  # pylint: disable=import-error
from tensorflow.contrib.layers import batch_norm  # pylint: disable=import-error
...
class Autoencoder(object):
    def __init__(self, inputDim, l2scale, compressDims, aeActivation, decompressDims, dataType):
        ...
        self.name = 'dpgan/fc/autoencoder'
...
class Generator(object):
    def __init__(self, randomDim, l2scale, generatorDims, generatorActivation, dataType):
    ...
    def __call__(self, z):
        ...
        with tf.variable_scope(self.name, regularizer=tcl.l2_regularizer(self.l2scale)):
            for i, genDim in enumerate(self.generatorDims[:-1]):
                ...
                tempDim = genDim
            W = tf.get_variable('W' + str(i), shape=[tempDim, self.generatorDims[-1]])  # pylint: disable=undefined-loop-variable
            h = tf.matmul(tempVec, W)
...
class buildDiscriminator(object):
    ...
    def __init__(self, inputDim, discriminatorDims, discriminatorActivation, decompressDims, aeActivation, dataType, l2scale):
        ...
        self.aeActivation = aeActivation
        self.dataType = dataType
        self.name = 'dpgan/fc/build_d_net'
